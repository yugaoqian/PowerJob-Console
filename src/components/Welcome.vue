	...
	<div id="container">
		...
		<div id="head">
			<el-row type="flex" class="row-bg" justify="end">
				<el-col span="2">
					<el-button type="primary" plain @click="appRegisterFormVisible = true">
		...
		<div id="welcome">
			...
			<!-- 登录表单 -->
			<el-form ref="loginForm" :rules="rules" :model="appLoginForm" label-width="0px" class="loginWrap">
				<el-form-item label-width="" prop="appName">
					<el-autocomplete :trigger-on-focus="false" class="loginWrap" v-model="appLoginForm.appName"
						@blur="appLoginForm.appName=$event.target.value.trim()" :fetch-suggestions="queryAppNames"
						:placeholder="$t('message.appName')" />
				</el-form-item>
				<el-form-item label-width="" prop="password">
					<el-input v-model="appLoginForm.password" @blur="appLoginForm.password=$event.target.value.trim()"
						:placeholder="$t('message.password')" type="password" show-password="true" />
				</el-form-item>
				<el-form-item>
			...
			<el-dialog :title="$t('message.appRegister')" :visible.sync="appRegisterFormVisible" width="35%">
				<el-form ref="registerForm" :model="appRegisterForm" :rules="rules1" style="margin:0 5px">

					<el-form-item :label="$t('message.appName')" prop="appName">
						<el-input v-model="appRegisterForm.appName"
							@blur="appRegisterForm.appName=$event.target.value.trim()" />
					</el-form-item>

					<el-form-item :label="$t('message.appPassword')" prop="password">
						<el-input v-model="appRegisterForm.password" type="password" show-password="true"
							@blur="appRegisterForm.password=$event.target.value.trim()" />
					</el-form-item>
	...
	export default {
		...
		data() {
			return {
				...
				rules1: {
					appName: [this.$rules.required, this.$rules.name()],
		...
		methods: {
			...
			registerApp() {
				...
				this.$refs['registerForm'].validate(valid => {
					if (valid) {
						let appRegisterInfo = {};
						appRegisterInfo.appName = this.appRegisterForm.appName;
						appRegisterInfo.password = this.$getRsaCode(this.appRegisterForm.password);
						this.axios.post("/appInfo/save", appRegisterInfo).then(() => {
			...
			login() {
				...
				this.$refs['loginForm'].validate(valid => {
					if (valid) {
						let appLoginInfo = {};
						appLoginInfo.appName = this.appLoginForm.appName;
						appLoginInfo.password = this.$getRsaCode(this.appLoginForm.password);
						this.axios.post("/appInfo/assert", appLoginInfo).then(res => {

							// 勾选了保持登录状态，就开启自动登录，直接本地存用户名密码（内部系统浏览器明文存问题不大）
							if (this.stayLogged) {
								window.localStorage.setItem('oms_auto_login', JSON.stringify(this
									.appLoginForm));
							}
